...
export default function AdminInternNonDisclosureClausesPage() {
  const qc = useQueryClient();

  const { data, isLoading } = useQuery<{ items: Clause[] }>({
    queryKey: ["/api/admin/intern/non-disclosure-clauses"],
  });

  const items = data?.items ?? [];
  const primaryClause = React.useMemo(() => {
  ...
  const [form, setForm] = React.useState({
    title: "Non-Disclosure of Contact Information",
    bodyHtml: "",
  });

  React.useEffect(() => {
    const nextTitle = primaryClause?.title ?? "Non-Disclosure of Contact Information";
    const nextBody = primaryClause?.bodyHtml ?? "";
    setForm({ title: nextTitle, bodyHtml: nextBody });
  }, [primaryClause?.id, primaryClause?.title, primaryClause?.bodyHtml]);

  const saveMutation = useMutation({
    mutationFn: async (payload: typeof form) => {
      if (primaryClause) {
        const res = await apiRequest("PUT", `/api/admin/intern/non-disclosure-clauses/${primaryClause.id}`, {
          title: payload.title,
          bodyHtml: payload.bodyHtml,
          sortOrder: primaryClause.sortOrder ?? 0,
          isActive: primaryClause.isActive ?? true,
        });
        return res.json();
      }

      const res = await apiRequest("POST", "/api/admin/intern/non-disclosure-clauses", {
        title: payload.title,
        bodyHtml: payload.bodyHtml,
        sortOrder: 0,
        isActive: true,
    ...
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["/api/admin/intern/non-disclosure-clauses"] });
    },
  });

  return (
    <AdminLayout
      title="Non-Disclosure Clauses"
      description="Manage the Non-Disclosure of Contact Information clauses shown for interns."
    >
      <div className="space-y-6">
        <SectionShell title="Non-Disclosure of Contact Information" description="This content is shown on the intern Non-Disclosure link.">
