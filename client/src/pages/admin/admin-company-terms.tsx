...
export default function AdminCompanyTermsPage() {
  const qc = useQueryClient();

  const { data, isLoading } = useQuery<{ terms: { title: string; bodyHtml: string } | null }>({
    queryKey: ["/api/admin/company/terms"],
  });
  ...
  React.useEffect(() => {
    const nextTitle = data?.terms?.title ?? "Company Terms and Conditions";
  ...
  const saveMutation = useMutation({
    mutationFn: async (payload: typeof form) => {
      const res = await apiRequest("PUT", "/api/admin/company/terms", {
        title: payload.title,
    ...
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["/api/admin/company/terms"] });
    },
  });

  return (
    <AdminLayout title="Company Terms" description="Edit the Company/Employer Terms & Conditions page content.">
      <div className="space-y-6">
        <SectionShell title="Company Terms & Conditions" description="This content is shown on the employer signup Terms link.">
