...
export default function EmployerAlertsPage() {
  ...
  const handleMarkAllRead = async () => {
    ...
    try {
      ...
      const res = await apiRequest(
        "POST",
        `/api/employer/${encodeURIComponent(employerId)}/notifications/read-all`,
        {},
      );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-emerald-50/20 to-teal-50/30">
      <EmployerHeader active="notifications" />
      ...
      <div className="container max-w-5xl mx-auto px-4 md:px-6 py-6 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" className="h-9 text-xs" onClick={() => setLocation("/employer/dashboard")}>
              Back
            </Button>
          </div>
        </div>
